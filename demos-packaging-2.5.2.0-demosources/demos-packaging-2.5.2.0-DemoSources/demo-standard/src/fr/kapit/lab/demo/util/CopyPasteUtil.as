...
public class CopyPasteUtil
{
	...
	public static function paste():void
	{
		...
		if((_clipboardNodesBuffer && _clipboardNodesBuffer.length>0) || (_clipboardGroupsBuffer && _clipboardGroupsBuffer.length>0))
		{
	...
	protected static function pasteNode(node:DiagramSprite):ISprite
	{
		...
		if(data is VCardData)
			newNode= _diagrammerModel.addNodeWithData(VCardData(data).clone(), VCardData(data).name, new Point(node.x+_offsetNode.x, node.y+_offsetNode.y),node.width,node.height);
		else if(data is UMLData)
			newNode= _diagrammerModel.addNodeWithData(UMLData(data).clone(), UMLData(data).classs, new Point(node.x+_offsetNode.x, node.y+_offsetNode.y),node.width,node.height);
		else if(data is PeopleData)
			newNode= _diagrammerModel.addNodeWithData(PeopleData(data).clone(), "People Icon", new Point(node.x+_offsetNode.x, node.y+_offsetNode.y),node.width,node.height);
		else // Case default data
		{
			newNode = _diagrammerModel.addNode(node.data.label, node.data.renderer, new Point(node.x+_offsetNode.x, node.y+_offsetNode.y), node.data.size);
	...
	protected static function pasteGroup(group:DiagramGroup):IGroup
	{
		...
		var newGroup:IGroup = _diagrammerModel.diagrammer.groupElements(copyGroupData(group.data), contentGroup);
		if(newGroup is DefaultGroupRenderer)
	...
	protected static function copyGroupData(groupData:Object):Object
	{
		var item:Object = new Object();
		if(groupData.hasOwnProperty('label'))
			item.label = groupData['label'];
		if(groupData.hasOwnProperty('layout'))
			item.layout = groupData['layout'];
		return item;
	}
