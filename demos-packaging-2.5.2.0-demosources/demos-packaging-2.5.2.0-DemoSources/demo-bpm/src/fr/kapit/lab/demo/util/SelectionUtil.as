...
public class SelectionUtil
{
	 ...
	 */
	public static function explodeList(list:Array, nodes:Array, groups:Array, edges:Array, nonDefaultNodes:Array):void
	{
		var sprite:ISprite;
		var link:ILink;
		...
		for each (var item:Object in list)
		{
			...
			if (null != sprite)
			{
				...
				else
				{
					...
					{
						nodes.push(sprite);
						if(!(sprite.itemRenderer is DefaultItemRenderer))
							nonDefaultNodes.push(sprite);
					}
				}
				continue;
			}
	...
	public static function extractNodes(list:Array, nodes:Array):void
	{
		var groups:Array = [];
		var edges:Array = [];
		var nodeDefaultNodes:Array = [];
		explodeList(list, nodes, groups, edges, nodeDefaultNodes);
	}
	 ...
	 * list of elements.
	 * "top most" means elements, from <code>list</code>, not contained
	 * by any other elements from <code>list</code>.
	 *
	 * @param list
	 * 		list of elements
	 * @return
	...
	public static function onlyTopMostElements(list:Array):Array
	{
		...
		var aSortedGroups:Array = groups.sortOn("subGroupDepth", Array.NUMERIC);
		var aSprites:Array = [];

		for each (var group:IGroup in aSortedGroups)
		{
			getSprites(group, aSprites);
		}
		for each (var sprite:Object in aSprites)
		{
